...
def get_commandes_by_devis(db: Session, devis_id: int) -> List[Commande]:
    return db.query(Commande).filter(Commande.devis_id == devis_id).all()
def get_commandes_by_status(db: Session, status: str) -> List[Commande]:
    return db.query(Commande).filter(Commande.statut == status).all()