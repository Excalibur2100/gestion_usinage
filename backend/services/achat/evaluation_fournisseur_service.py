...
from backend.db.models.tables.achat.evaluation_fournisseur import EvaluationFournisseur
...
def search_evaluations(db: Session, filters: EvaluationFournisseurSearch, skip: int = 0, limit: int = 50):
    ...
    if filters.statut:
        query = query.filter(EvaluationFournisseur.statut == filters.statut)
    if filters.type_evaluation:
        query = query.filter(EvaluationFournisseur.type_evaluation == filters.type_evaluation)
    if filters.date_min:
...
def export_evaluations_csv(db: Session) -> io.StringIO:
    ...
    for e in db.query(EvaluationFournisseur).all():
        writer.writerow([
            ...
            e.statut,
            e.type_evaluation,
            e.date_evaluation.strftime("%Y-%m-%d") if getattr(e, "date_evaluation", None) is not None else "",
            e.commentaire or ""
        ])
    buffer.seek(0)
    return buffer
